// --- Gradients 1.1 Multiplayer (M5Unified + M5GFX + ESP-NOW) ---
...
// True multiplayer v1.4: host + up to 3 clients.
...
const uint16_t PLAYER_COLORS[MAX_PLAYERS] = { TFT_RED, TFT_BLUE, TFT_CYAN, TFT_MAGENTA }; // 0xFD20 = orange
...
void drawGameOverOverlay(int8_t winner, bool localIsHost) {
  (void)localIsHost; // winner is now compared against localPlayerId
  ...
  M5.Display.drawString("GAME OVER", cx, cy - 24);
  ...
  if (winner == 0) {
    ...
    int winPid = winner - 1;
    if (winPid == localPlayerId) msg = "You win!";
    else if (winPid >= 0 && winPid < MAX_PLAYERS) msg = String("Winner: ") + PLAYER_LABELS[winPid];
    else msg = "Game over";
  }
  M5.Display.setTextColor(TFT_WHITE, TFT_BLACK);
  M5.Display.drawString(msg, cx, cy + 4);
  M5.Display.drawString("Hold A to restart", cx, cy + 32);
  M5.Display.drawString("Auto off in 60s", cx, cy + 56);
  M5.Display.drawString(batteryStatusString(), cx, cy + 78);
}
...
void drawModeMenu(bool touch) {
  ...
  drawBlackTextOnly(menuScreen, "Gradients 1.1", 10, 10);
...
void setup() {
  ...
  while (true) {
    ...
    if (nowMenuMs - menuIdleStartMs >= MENU_AUTO_POWER_OFF_MS) {
      autoPowerOffNow("Menu timeout");
    }

    if (chooseSingle && cfgIsHostDevice) {
      multiplayer = false;
      isHost = true;
      goto mode_selected;
    }
...
void loopHostLike(bool asMultiplayer) {
  ...
  // In multiplayer, paired clients become active after their first input packet.
  // This avoids ghost balls from paired-but-offline devices, while still keeping
  // already joined players active during short packet dropouts.
  if (asMultiplayer) {
    playerActiveMask = 0x01;
    for (int slot = 0; slot < MAX_CLIENTS; slot++) {
      uint8_t pid = slot + 1;
      if (clientMacValid[slot] && pLastInputMs[pid] != 0) playerActiveMask |= (1 << pid);
    }
  } else {
    playerActiveMask = 0x01;
  }
  ...
  if (millis() > gameEndTimeMs) {
    ...
    while (true) {
      M5.update();
      if (M5.BtnA.pressedFor(800)) esp_restart();
      if (millis() - postGameScreenStartMs >= POSTGAME_AUTO_POWER_OFF_MS) {
...
void loopClient() {
  ...
  if (remoteGameOver) {
    if (M5.BtnA.pressedFor(800)) esp_restart();
    if (postGameScreenStartMs != 0 && millis() - postGameScreenStartMs >= POSTGAME_AUTO_POWER_OFF_MS) {
      autoPowerOffNow("Post-game timeout");
    }
  }

  if (firstStateReceived && !remoteGameOver) {
    if (millis() - lastStateMs > 150) {
      drawFrame(targX, targY, 0);
      lastStateMs = millis();
    }
  }
